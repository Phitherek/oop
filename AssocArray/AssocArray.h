...
public:
	...
	void insert(std::string key, int value); ///< A function, that inserts a value to associative array.
	/// \param key A key which will identify the entry, of type std::string.
	/// \param value An int value to be inserted.
	void remove(std::string key); ///< A function, that removes a value from associative array.
	...
	int& operator[](std::string key); ///< An operator: [], which can be used to extract integer values by their keys from associative array.
	/// \param key A key which identifies the entry.
	/// \return An int value associated with given key.
	int size(); ///< A function, that returns size of the associative array.
